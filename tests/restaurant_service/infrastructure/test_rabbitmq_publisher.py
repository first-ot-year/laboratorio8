"""Unit tests for RabbitMQDinnerPublisher (pika mocked)."""
...
from restaurant_service.infrastructure.rabbitmq_publisher import RabbitMQDinnerPublisher
...
class TestPublishSuccess:
    def test_publishes_message_to_correct_exchange(self, mock_connection):
        connection, channel = mock_connection
        with patch("pika.BlockingConnection", return_value=connection):
            RabbitMQDinnerPublisher(_SETTINGS).publish(_EVENT)
        channel.basic_publish.assert_called_once()
        call_kwargs = channel.basic_publish.call_args.kwargs
        assert "restaurant" in call_kwargs["exchange"] or call_kwargs["exchange"]
    ...
    def test_declares_exchange_and_queue(self, mock_connection):
        connection, channel = mock_connection
        with patch("pika.BlockingConnection", return_value=connection):
            RabbitMQDinnerPublisher(_SETTINGS).publish(_EVENT)
...
class TestPublishFailure:
    def test_amqp_error_raises_publish_error(self):
        with patch("pika.BlockingConnection", side_effect=AMQPError("refused")):
            with pytest.raises(PublishError, match="could not publish dinner event"):
                RabbitMQDinnerPublisher(_SETTINGS).publish(_EVENT)
